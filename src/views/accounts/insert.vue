    ...
    methods: {
        validated() {
           ...
           if(this.account.password != this.repass) {
            this.errors.password = "Mật Khẩu chưa khớp"
            isValid=false
           }
        ...
        save() {
            if(this.validated()) {
             this.$request.post('http://localhost:8082/api/create-account', 
             this.account).then(res=> {
                if(res.data.success) {
                    this.$swal.fire('Thêm mới tài khoản thành công','','sucsess')
                    this.$router.push({name: 'accountList'})
                } else {
                    alert('không thể thêm mới,email đã tồn tại')
                }
              })
            }
        },
